...
\lstinputlisting[language=Python, firstline=26, lastline=27]{linearregression.py}
%glob.glob(curr)
...
%we shall not forget to insert table.
...
%\begin{figure}[htbp] 
 % \centering
 %    \includegraphics[width=0.3\textwidth]{regression_number_of_travels}
 % \caption{Variable explorer in Python}
 % \label{fig: figure1}
%\end{figure}
We can now see the relation between the variables. In a last step we conduct the coefficients, the mean sqare error and the variance:
\begin{1stlisting}
print('Coefficients: \n', regr.coef_)
